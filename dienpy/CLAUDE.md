...
Personal modules (medroy, nightvid, calendar, etc.) have moved to `hyppy` in the hypothalamus repo.

## Architecture

- **Entry point**: `dienpy/__main__.py` — discovers and dispatches to submodules by name
- **Each module** is self-contained with a `main()` function and optionally a `get_completions(args: list[str]) -> list[str]` function for shell tab-completion
- **Shell completion**: `dotfiles/.local/share/bash-completion/completions/dienpy` calls `dienpy --complete [module] [args...]`
